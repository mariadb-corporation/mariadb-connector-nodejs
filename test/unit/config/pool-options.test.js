...
describe.concurrent('test pool options', () => {
  test('with options', () => {
    const result = new PoolOptions(
      'mariadb://root:pass@example.com:3307/db?metaAsArray=false&ssl=true&dateStrings=true&resetAfterUse=false' +
        '&acquireTimeout=200&connectionLimit=2&minDelayValidation=100&minimumIdle=2&noControlAfterUse=true' +
  ...
  test('with other options', () => {
    const result = new PoolOptions(
      'mariadb://root:pass@example.com:3307/db?metaAsArray=true&ssl=false&dateStrings=false&resetAfterUse=true' +
        '&acquireTimeout=800&connectionLimit=2&minDelayValidation=100&minimumIdle=2&noControlAfterUse=false' +
