...
describe("test PacketInputStream data", function() {
  ...
  it("small complete packet", function() {
    let conn = {
  ...
  it("small packet multi part header", function() {
    let conn = {
      _dispatchPacket: packet => {
        assert.deepEqual(Buffer.from([1, 2, 3, 4, 5]), packet._toBuf());
      }
  ...
  it("small packet multi part header 2", function() {
    let conn = {
      _dispatchPacket: packet => {
        assert.deepEqual(Buffer.from([1, 2, 3, 4, 5]), packet._toBuf());
      }
  ...
  it("small packet multi part header 3", function() {
    let conn = {
      _dispatchPacket: packet => {
        assert.deepEqual(Buffer.from([1, 2, 3, 4, 5]), packet._toBuf());
      }
  ...
  it("small packet multi part header 4", function() {
    let conn = {
      _dispatchPacket: packet => {
        assert.deepEqual(Buffer.from([1, 2, 3, 4, 5]), packet._toBuf());
      }
  ...
  it("small packet multi part data", function() {
    let conn = {
      _dispatchPacket: packet => {
        assert.deepEqual(Buffer.from([1, 2, 3, 4, 5]), packet._toBuf());
      }
  ...
  it("big packet multi part data", function() {
    this.timeout(90000); //can take some time
  ...
  it("big packet multi part data with part", function() {
    this.timeout(90000); //can take some time
    let beenDispatch = false;
    let conn = {
      _dispatchPacket: packet => {
