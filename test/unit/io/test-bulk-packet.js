...
describe("bulk packet", () => {
  ...
  it("writeDefaultLengthEncodedString", () => {
    const conOpts = new ConnOptions(baseOpts);
    let out = new PacketOutputStream(conOpts, null);
    ...
    assert.isTrue(stream.bufs[0].slice(prevPos + 9, MAX_BUFFER_SIZE).equals(Buffer.from(str.substring(0, MAX_BUFFER_SIZE - (prevPos + 9)))));
    assert.isTrue(packet.buf.slice(4, packet.pos).equals(Buffer.from(str.substring(MAX_BUFFER_SIZE - (prevPos + 9)))));
