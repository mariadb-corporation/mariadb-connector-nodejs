...
describe('NO_BACKSLASH_ESCAPES escaping', () => {
  ...
  ['utf8mb4', 'latin1'].forEach((charset) => {
    test(`${charset}: quote is doubled, backslash left alone`, () => {
      assert.equal(escapedString(charset, NBE, "a'b"), "'a''b'");
      assert.equal(escapedString(charset, NBE, 'a\\b'), "'a\\b'");
      assert.equal(escapedString(charset, NBE, "a\\'b"), "'a\\''b'");
    });

    test(`${charset}: default mode still escapes with backslash`, () => {
      assert.equal(escapedString(charset, 0, "a'b"), "'a\\'b'");
      assert.equal(escapedString(charset, 0, 'a\\b'), "'a\\\\b'");
    });

    test(`${charset}: injection payload cannot close the literal`, () => {
  ...
  test('long string (buffer path) is escaped with doubled quotes', () => {
  ...
  test('conn.escape() honours the flag', () => {
