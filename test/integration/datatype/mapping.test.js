...
describe.concurrent('mapping', () => {
  ...
  test('query mapping field', async ({ skip }) => {
    if (!shareConn.info.isMariaDB() && !shareConn.info.hasMinVersion(5, 6)) {
  ...
  test('query null mapping field', async () => {
    const rows = await shareConn.query('SELECT * FROM nullMappingTable');
    for (let i = 0; i < 27; i++) {
      assert.equal(rows[0]['t' + (i + 1)], null);
    }
  });

  test('dataType with null', async () => {
    await shareConn.query('DROP TABLE IF EXISTS dataTypeWithNull');
    await shareConn.query(
      'CREATE TABLE dataTypeWithNull (id int not null primary key auto_increment, test longblob, ' +
        'test2 blob, test3 text)'
    );
    await shareConn.beginTransaction();
