...
describe.concurrent('geometry data type', () => {
  ...
  test('geometry escape', async function () {
    let prefix =
      shareConn.info &&
  ...
  test('LineString format', async ({ skip }) => {
  ...
  test('Polygon format', async ({ skip }) => {
  ...
  test('MultiPoint format', async ({ skip }) => {
    //ST_MultiPointFromText alias doesn't exist before 10.1.4 / 5.7.6
    if (!shareConn.info.isMariaDB() && !shareConn.info.hasMinVersion(5, 7, 6)) {
      skip();
      return;
    }
    if (shareConn.info.isMariaDB() && !shareConn.info.hasMinVersion(10, 1, 4)) {
      skip();
      return;
    }
  ...
  test('MultiPoint insert', async ({ skip }) => {
    ...
    const expected = [
      ...
      {
        g: serverPermitExtendedMetas ? { type: 'MultiPoint' } : null
      },
      {
        g: serverPermitExtendedMetas ? { type: 'MultiPoint' } : null
      }
  ...
  test('Multi-line String format', async ({ skip }) => {
    ...
    await shareConn.query(
      'INSERT INTO gis_multi_line VALUES\n' +
        ...
        '    (ST_MLineFromWKB(ST_AsWKB(MultiLineString(LineString(Point(1, 2), Point(3, 5)), ' +
  ...
  test('Multi-line insert', async ({ skip }) => {
    ...
    const expected = [
      ...
      {
        g: serverPermitExtendedMetas ? { type: 'MultiLineString' } : null
      },
      {
        g: serverPermitExtendedMetas ? { type: 'MultiLineString' } : null
      },
  ...
  test('Multi-polygon format', async ({ skip }) => {
    ...
    await shareConn.query(
      'INSERT INTO gis_multi_polygon VALUES\n' +
        "    (ST_MultiPolygonFromText('MULTIPOLYGON(((28 26,28 0,84 0,84 42,28 26),(52 18,66 23,73 9,48 6,52 18))," +
        "((59 18,67 18,67 13,59 13,59 18)))')),\n" +
        "    (ST_MPolyFromText('MULTIPOLYGON(((28 26,28 0,84 0,84 42,28 26),(52 18,66 23,73 9,48 6,52 18))," +
        "((59 18,67 18,67 13,59 13,59 18)))')),\n" +
        '    (ST_MPolyFromWKB(ST_AsWKB(MultiPolygon(Polygon(LineString(Point(0, 3), Point(3, 3), Point(3, 0), ' +
        'Point(0, 3)))))))'
    );
    const expected = [
      {
  ...
  test('Multi-polygon insert', async ({ skip }) => {
    ...
    const expected = [
      ...
      {
        g: serverPermitExtendedMetas ? { type: 'MultiPolygon' } : null
      },
      {
        g: serverPermitExtendedMetas ? { type: 'MultiPolygon' } : null
      },
  ...
  test('Geometry collection format', async ({ skip }) => {
