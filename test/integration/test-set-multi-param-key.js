...
describe('permitSetMultiParamEntries key escaping', () => {
  ...
  before(async () => {
    conn = await base.createConnection({ permitSetMultiParamEntries: true });
  ...
  it('a backtick in a key cannot write another column', async () => {
  ...
  it('escape() keeps the key inside a single identifier', () => {
    assert.equal(conn.escape(MALICIOUS), "`secret`` = 'PWNED-BY-KEY-INJECTION', ``name`='ignored'");
  });

  it('ordinary keys still expand to columns', async () => {
    await conn.query('DELETE FROM set_key_inj');
  ...
  it('a key needing quoting (reserved word) still works', async () => {
