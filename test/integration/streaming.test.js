...
describe.sequential('streaming', () => {
  ...
  beforeAll(async () => {
    ...
    await shareConn.query(
      'CREATE TABLE Streaming (id int NOT NULL AUTO_INCREMENT, b longblob, c varchar(10), d longblob, e varchar(10), ' +
        'PRIMARY KEY (id))'
    );
  ...
  test('Streaming url content', async () => {
    await shareConn.query('DROP TABLE IF EXISTS StreamingContent');
    await shareConn.query(
      'CREATE TABLE StreamingContent (id int NOT NULL AUTO_INCREMENT, b longblob, c' + ' varchar(10), PRIMARY KEY (id))'
    );
  ...
  test('Streaming single parameter', async ({ skip }) => {
    if (maxAllowedSize <= size) return skip();
  ...
  test('Streaming multiple parameter', async ({ skip }) => {
    if (maxAllowedSize <= size) return skip();
  ...
  test('Streaming multiple parameter begin no stream', async ({ skip }) => {
    if (maxAllowedSize <= size) return skip();
    const r = fs.createReadStream(halfFileName);
    const r2 = fs.createReadStream(halfFileName);
    await shareConn.query('truncate Streaming');
  ...
  test('Streaming multiple parameter ensure max callstack', async ({ skip }) => {
    if (maxAllowedSize <= size) return skip();
    const r = fs.createReadStream(halfFileName);
