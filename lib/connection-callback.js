...
function ConnectionCallback(options) {
  ...
  const preparePromise = this.prepare.bind(this);
  const executePromise = this.execute.bind(this);
  ...
  const _connectCallback = function (callback) {
    if (!callback) {
      throw new Errors.createError('missing callback parameter', Errors.ER_MISSING_PARAMETER, this.info);
    }
    ...
    if (connecting === 1) {
      ...
      switch (this._status()) {
        ...
        case Status.CLOSED:
          callback(
            Errors.createError('Connection closed', Errors.ER_CONNECTION_ALREADY_CLOSED, this.info, '08S01', null, true)
          );
          break;
  ...
  const _prepareCallback = (sql, cb) => {
    ...
    preparePromise(sql)
      .then((prepare) => {
        prepare.execute = prepare._executeCallback;
        if (_cb) _cb(null, prepare, null);
      })
      .catch(_cb || emptyError);
  };

  const _executeCallback = (sql, values, cb) => {
    let _values = values,
      _cb = cb;
  ...
  this.prepare = _prepareCallback;
  this.execute = _executeCallback;
