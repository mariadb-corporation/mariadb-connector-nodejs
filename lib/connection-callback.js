...
import { rethrowUncaught } from './misc/utils.js';
...
class ConnectionCallback {
  ...
  static _HANDLE_NEW_ROWS(row) {
    this._rows[this._responseIndex].push(row);
    if (this.listenerCount('data') > 0) this.emit('data', row);
  }
  ...
  static _QUERY_CMD(conn, cmdParam) {
    ...
    // Emitting a `data` event per row lets callers stream a big result-set via
    // `conn.query(...).on('data', ...)`. Most callers only use the callback, so
    // skip the per-row emit unless a `data` listener is actually attached - the
    // emit is otherwise pure overhead on the hot row-parsing path. Streaming via
    // `queryStream()` replaces `handleNewRows` in `_stream()`, so it is unaffected.
    cmd.handleNewRows = ConnectionCallback._HANDLE_NEW_ROWS;
  ...
  ping(timeout, callback) {
    ...
    if (this.#conn.opts.trace) Error.captureStackTrace(cmdParam);
    new Promise(this.#conn.ping.bind(this.#conn, cmdParam))
      .then(
        () => {
          if (_cb) _cb(null);
        },
  ...
  connect(callback) {
    ...
    switch (this.#conn.status) {
      ...
      case Status.TERMINATING_SOCKET:
      case Status.CLOSED:
        callback.call(
          this,
          Errors.createError(
