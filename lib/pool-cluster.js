...
function PoolCluster(args) {
   ...
   * @private
   */
  const _getConnectionCallback = (pattern, selector, callback, avoidNodeKey) => {
    const matchingNodeList = _matchingNodes(pattern || /^/);
    ...
    const retry = _getConnectionCallback.bind(this, pattern, selector, callback);
    try {
      const nodeKey = _selectPool(matchingNodeList, selector, avoidNodeKey);
      _handleConnectionCallbackError(matchingNodeList, nodeKey, retry, callback);
    } catch (e) {
      callback(e);
    }
  };
   ...
   * @private
   */
  const _handleConnectionCallbackError = (nodeList, nodeKey, retryFct, callback) => {
    const node = nodes[nodeKey];
