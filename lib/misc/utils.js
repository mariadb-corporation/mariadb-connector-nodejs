  ...
  switch (typeof value) {
    ...
    case 'object':
      if (Object.prototype.toString.call(value) === '[object Date]') {
        ...
        return "_binary'" + escapeString(info, stValue) + "'";
      } else if (typeof value.toSqlString === 'function') {
        return "'" + escapeString(info, String(value.toSqlString())) + "'";
        ...
        if (
          ...
          if (opts.permitSetMultiParamEntries) {
            ...
            for (let key in value) {
              ...
              out += TextEncoder.escapeId(key) + '=';
              out += escapeParameters(opts, info, val);
            }
            if (out === '') return "'" + escapeString(info, JSON.stringify(value)) + "'";
            return out;
          } else {
            return "'" + escapeString(info, JSON.stringify(value)) + "'";
          }
        }
      }
    default:
      return "'" + escapeString(info, value) + "'";
  }
}
...
// with NO_BACKSLASH_ESCAPES, backslash is an ordinary character and the quote
// is escaped by doubling it, so it is the only character needing escaping.
const QUOTE_GLOBAL_REGEXP = /'/g;
const QUOTE_ESCAPE = { "'": "''" };

const escapeString = (info, val) => {
  // info can be null when a pool has no connection yet to read the status from
  const noBackslashEscapes = info != null && (info.status & ServerStatus.STATUS_NO_BACKSLASH_ESCAPES) !== 0;
  const escapeRegexp = noBackslashEscapes ? QUOTE_GLOBAL_REGEXP : CHARS_GLOBAL_REGEXP;
  const escapeMap = noBackslashEscapes ? QUOTE_ESCAPE : LITTERAL_ESCAPE;
  let offset = 0;
  let escaped = '';
  let match;

  while ((match = escapeRegexp.exec(val))) {
    escaped += val.substring(offset, match.index);
    escaped += escapeMap[match[0]];
    offset = escapeRegexp.lastIndex;
  }
...
const rethrowUncaught = (err) => {
  process.nextTick(() => {
    throw err;
  });
};

export { log, toHexString, escapeId, escapeParameters as escape, rethrowUncaught };
