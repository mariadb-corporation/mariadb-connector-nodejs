...
class ConnectionInformation {
  #redirectFct;
  #fatalErrorFct;
  constructor(opts, redirectFct, fatalErrorFct) {
    this.threadId = -1;
    this.status = null;
    ...
    this.#fatalErrorFct = fatalErrorFct;
    this.redirectRequest = null;
  }

  /**
   * Close the connection on an unrecoverable error, dispatching it to the pending commands.
   * Permits the packet reader, or a command detecting a protocol violation, to tear down the
   * connection instead of leaving a desynchronized stream behind.
   *
   * @param err               the fatal error
   * @param avoidThrowError   when the caller reports the error itself
   */
  fatalError(err, avoidThrowError) {
    if (this.#fatalErrorFct) this.#fatalErrorFct(err, avoidThrowError);
  }
