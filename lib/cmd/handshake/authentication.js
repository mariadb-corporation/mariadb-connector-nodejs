...
class Authentication extends Command {
  ...
  /**
   * Whether the server certificate must be validated when processing the resulting OK/ERR packet.
   *
   * Certificate validation (self-signed fingerprint check or server identity check) is tied to the
   * TLS handshake, which only happens on the initial connection. COM_CHANGE_USER reuses the already
   * validated TLS session, and the server does not resend a fingerprint hash, so subclasses handling
   * it (see {@link ChangeUser}) skip validation.
   *
   * @returns {boolean} true if server certificate validation must be performed
   */
  validateServerCert() {
    return true;
  }
  ...
  handshakeResult(packet, out, opts, info) {
    ...
    switch (marker) {
      ...
      case 0x00:
        ...
        if (info.requireValidCert && this.validateServerCert()) {
          if (info.selfSignedCertificate) {
            ...
            if (packet.remaining()) {
              ...
              if (validationHash.length > 0) {
                if (!this.plugin.isMitmProof() || !Boolean(this.cmdParam.opts.password)) {
      ...
      case 0xff:
        ...
        authErr.fatal = true;
        if (info.useFingerprintValidation && info.selfSignedCertificate && this.validateServerCert()) {
          // TLS was forced to trust, and certificate validation is required
  ...
  validateFingerPrint(validationHash, info) {
    ...
    // SHA256 is mandated by the server's challenge-response auth protocol (combines the
    // password hash with the server seed and TLS fingerprint); this is not password-at-rest
    // storage, so a slow KDF (bcrypt/scrypt/argon2) does not apply.
    ...
    // constant-time comparison: serverValidationHex is derived from a secret token, so a
    // short-circuiting compare (===) would leak its matching prefix length to a MitM.
    const expected = Buffer.from(hashHex, 'ascii');
    const actual = Buffer.from(serverValidationHex, 'ascii');
  ...
  dispatchAuthSwitchRequest(packet, out, opts, info) {
    ...
    try {
      ...
      if (
        ...
        this.validateServerCert() &&
        Boolean(this.cmdParam.opts.password) &&
        !this.plugin.isMitmProof()
      ) {
        return this.throwNewError(
