    ...
    if (opts.ssl) {
      if (info.serverCapabilities & Capabilities.SSL) {
        ...
        this.auth._createSecureContext(info, () => {
          ...
          if (info.requireValidCert && info.selfSignedCertificate && Boolean(opts.password) && !this.permitHash()) {
            return this.throwNewError(
              `Unsupported authentication plugin ${this.pluginName} with Self signed certificates. Either set 'ssl: { rejectUnauthorized: false }' (trust mode) or provide server certificate to client`,
              true,
              info,
              '08000',
