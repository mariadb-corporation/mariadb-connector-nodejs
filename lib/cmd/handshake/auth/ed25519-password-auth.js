...
class Ed25519PasswordAuth extends PluginAuth {
  ...
  static encryptPassword(password, seed) {
    ...
    const d = privateScalar(password);
  ...
  /**
   * Server-side authentication data for this user: the Ed25519 public key derived from the
   * password. This is what the server stores (base64 encoded) in its user table, so it is the
   * shared secret both sides combine with the seed and the certificate fingerprint to
   * authenticate a self-signed certificate.
   *
   * @param conf  connection options holding the password
   * @returns {Buffer} the 32-byte raw public key
   */
  hash(conf) {
    return publicKey(privateScalar(conf.password));
  }
}

/**
 * Derive the Ed25519 private scalar from a password, per the MariaDB ed25519 plugin: the
 * SHA-512 of the password, clamped as RFC 8032 mandates.
 *
 * @param password  user password
 * @returns {Buffer} 64-byte digest whose first half is the clamped scalar
 */
function privateScalar(password) {
  // Protocol-mandated, not password-at-rest storage, so a slow KDF (bcrypt/scrypt/argon2)
  // does not apply.
  const hash = Crypto.createHash('sha512');
  // codeql[js/insufficient-password-hash]
  const d = hash.update(Buffer.from(password)).digest();
  d[0] &= 248;
  d[31] &= 127;
  d[31] |= 64;
  return d;
...
/**
 * Ed25519 public key for a private scalar: the scalar multiplied by the curve base point.
 *
 * @param d  private scalar, as returned by {@link privateScalar}
 * @returns {Buffer} the 32-byte packed public key
 */
...
export default Ed25519PasswordAuth;
