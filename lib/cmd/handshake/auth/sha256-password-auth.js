...
class Sha256PasswordAuth extends PluginAuth {
        ...
        if (opts.rsaPublicKey) {
          try {
            ...
            if (!key.includes('-----BEGIN')) {
              // opts.rsaPublicKey is a client-configured path to the RSA public key,
  ...
  static encryptSha256Password(password, seed) {
    ...
    // Protocol-mandated challenge-response scramble (server seed mixed in), not
    // password-at-rest storage, so a slow KDF (bcrypt/scrypt/argon2) does not apply.
...
export default Sha256PasswordAuth;
