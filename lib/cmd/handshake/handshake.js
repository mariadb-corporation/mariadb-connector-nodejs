...
class Handshake extends Command {
  constructor(authenticationEnd, _createSecureContext, _addCommand, getSocket) {
    super();
    this.authenticationEnd = authenticationEnd;
  ...
  authEnded(err) {
    ...
    this.authenticationEnd(err);
  }

  defaultAuthSwitchHandler() {
    return (pluginName, packSeq, pluginData, info, opts, out, authenticationEnd) => {
      let pluginAuth;
      switch (pluginName) {
        ...
        default:
          ...
          authenticationEnd(err);
          return;
      }
      const plugin = new pluginAuth(packSeq, pluginData, authenticationEnd);
