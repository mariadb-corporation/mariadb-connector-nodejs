        ...
        case 'Polygon':
          if (value.coordinates && Array.isArray(value.coordinates)) {
            ...
            for (let i = 0; i < numRings; i++) {
              if (!Array.isArray(value.coordinates[i])) return null;
      ...
      switch (headerType) {
        ...
        case 'MultiPolygon':
          if (value && Array.isArray(value)) {
            ...
            let size = 0;
            for (let i = 0; i < numRings; i++) {
              // reject non-array rings before reserving space for them, so no byte
              // of the allocation can be left uninitialized by the writing loop
              if (!Array.isArray(value[i])) return null;
