...
class BatchBulk extends Parser {
  ...
  sendComStmtBulkExecute(out, opts, info) {
    ...
    main_loop: while (true) {
      ...
      // always SEND_TYPES_TO_SERVER + SEND_UNIT_RESULTS if possible
  ...
  success(initVal) {
    ...
    if (!this.sending && this.bulkPacketNo === 0) {
      ...
      if (this.firstError) {
        ...
        if (this._rows[0][0]['Affected_rows'] !== undefined) {
          // in case of bulk sending unit results
          if (!this.cmdOpts || this.cmdOpts.fullResult === undefined || this.cmdOpts.fullResult === true) {
            const rs = [];
        ...
        } else if (this._rows[0].affectedRows !== undefined) {
