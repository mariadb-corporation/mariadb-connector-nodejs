...
class Parser extends Command {
  ...
  readResultSet(packet, info) {
    ...
    this._getValue = this.opts.typeCast ? this.readCastValue : this.decoderReadRowData;
  ...
  readPrepareColumnsPacket(packet, out, opts, info) {
    this.columnNo--;
    this._columnsPrepare.push(new ColumnDefinition(packet, info));
    if (this.columnNo === 0) {
  ...
  parseRowAsArray(columns, packet, connOpts) {
    ...
    const nullBitMap = this.decoderNewRow(packet, columns);
    for (let i = 0; i < this._columnCount; i++) {
      row[i] = this._getValue(i, columns[i], this.opts, connOpts, packet, nullBitMap);
    }
    return row;
  }
  ...
  readCastValue(index, column, opts, connOpts, packet, nullBitmap) {
    this.decoderCastWrapper(index, column, opts, connOpts, packet, nullBitmap);
    return opts.typeCast(
      column,
      this.decoderReadRowData.bind(this, index, column, opts, connOpts, packet, nullBitmap)
    );
  }
