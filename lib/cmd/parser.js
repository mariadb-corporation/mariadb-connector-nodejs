...
import { validateFileName } from '../misc/parse.js';
import * as BinaryDecoder from './decoder/binary-decoder.js';
import * as TextDecoder from './decoder/text-decoder.js';
import OkPacket from './class/ok-packet.js';
...
class Parser extends Command {
  ...
  readResultSet(packet, info) {
    ...
    if (!this.validateColumnCount(this._columnCount, info)) return;

    this._rows.push([]);
    if (this.canSkipMeta && info.serverPermitSkipMeta && packet.readUInt8() === 0) {
      // Command supports skipping metadata, and Server tells that no columns follow, using prepare results
  ...
  /**
   * Validate the column count announced by the server before anything is allocated for it.
   *
   *
   * @param {number} columnCount - column count announced by the server
   * @param {Object} info - Connection information
   * @returns {boolean} true when the count is usable, false when the command has been rejected
   */
  validateColumnCount(columnCount, info) {
    if (columnCount >= 1 && columnCount <= this.opts.maxAllowedColumns) return true;

    // a count below 1 (the 8-byte encoding is read as a signed integer) would never be reached while
    // reading column packets, leaving the command waiting for a result-set that cannot complete.
    const err = Errors.createFatalError(
      columnCount >= 1
        ? `Server announced ${columnCount} columns, exceeding maxAllowedColumns (${this.opts.maxAllowedColumns}). ` +
            'Metadata is refused to prevent unbounded memory allocation.'
        : `Server announced an invalid column count (${columnCount})`,
      Errors.client.ER_MAX_ALLOWED_COLUMNS,
      info,
      '08S01',
  ...
  checkDuplicates() {
    if (this.opts.checkDuplicate) {
      ...
      for (let i = 0; i < this._columnCount; i++) {
        ...
        if (seen.has(header)) {
          this.throwUnexpectedError(
            `Error in results, duplicate field name \`${header}\`.\n(see option \`checkDuplicate\`)`,
            false,
            null,
            '42000',
            Errors.client.ER_DUPLICATE_FIELD
          );
          return;
        }
        seen.add(header);
      ...
      for (let i = 0; i < this._columnCount; i++) {
        for (let j = 0; j < i; j++) {
          if (this.tableHeader[j][0] === this.tableHeader[i][0] && this.tableHeader[j][1] === this.tableHeader[i][1]) {
            this.throwUnexpectedError(
              `Error in results, duplicate field name \`${this.tableHeader[i][0]}\`.\`${
    ...
    for (let i = 0; i < this._columnCount; i++) {
      if (privateFields.has(this.tableHeader[i][0])) {
        this.throwUnexpectedError(
          ...
          null,
          '42000',
          Errors.client.ER_PRIVATE_FIELDS_USE
        );

        // Continue parsing results to keep the connection state
  ...
  readLocalInfile(packet, out, opts, info) {
    ...
    if (!opts.permitLocalInfile) {
      ...
      const error = Errors.createError(
        'Cannot send local file when `permitLocalInfile` option is disabled. ' +
          'The server requested a local file although the capability was not advertised. ' +
          'Check for malicious server / proxy',
        Errors.client.ER_LOCAL_INFILE_DISABLED,
        info,
        'HY000',
        this.sql
      );
    ...
    if (!validateFileName(this.sql, this.initialValues, fileName)) {
      out.writeEmptyPacket();
      const error = Errors.createError(
        ...
        Errors.client.ER_LOCAL_INFILE_WRONG_FILENAME,
        info,
        'HY000',
        this.sql
      );
    ...
    let stream;
    try {
      // fileName was validated by validateFileName() against the client's own LOAD DATA
      // query and is gated by permitLocalInfile, so it cannot be an arbitrary server path.
      ...
      const error = Errors.createError(
        ...
        Errors.client.ER_LOCAL_INFILE_NOT_READABLE,
        info,
        '22000',
        this.opts.logParam ? this.displaySql() : this.sql
      );
