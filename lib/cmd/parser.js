...
class Parser extends Command {
  ...
  parseRowAsArray(columns, packet) {
    ...
    for (let i = 0; i < this._columnCount; i++) {
      row[i] = this._parseFonction[i].call(null, packet, i, nullBitMap, this.opts, this.unexpectedError);
    }
    return row;
  }
  ...
  parseRowStdText(columns, packet) {
    const row = {};
    for (let i = 0; i < this._columnCount; i++) {
      row[this.tableHeader[i]] = this._parseFonction[i].call(null, packet, i, null, this.opts, this.unexpectedError);
    }
    return row;
  }
  ...
  readCastValue(column, packet, index, nullBitmap, opts) {
    ...
    return opts.typeCast(
      column,
      column.__parser(this.binary, opts).bind(column, packet, index, nullBitmap, opts, this.unexpectedError)
    );
  }
