  ...
  constructor(packet, info, opts, binary) {
    ...
    if (binary) {
      switch (this.columnType) {
        case FieldType.TINY:
          if (this.signed()) {
            this.read = (packet, index, nullBitmap) =>
        ...
        case FieldType.BIGINT:
          this.read = (packet, index, nullBitmap, opts) => {
      ...
      switch (this.columnType) {
        ...
        case FieldType.INT24:
        case FieldType.YEAR:
          this.read = (packet) => packet.readIntLengthEncoded();
          break;

        case FieldType.FLOAT:
        case FieldType.DOUBLE:
          this.read = (packet) => packet.readFloatLengthCoded();
          break;
        ...
        case FieldType.TIME:
          this.read = (packet) => packet.readAsciiStringLengthEncoded();
          break;

        case FieldType.GEOMETRY:
          this.read = (packet) => packet.readGeometry(this.dataTypeName);
          break;

        case FieldType.JSON:
          //for mysql only => parse string as JSON object
          this.read = (packet) => JSON.parse(packet.readStringLength());
          break;
        ...
        default:
          ...
          if (this.collation.index === 63) {
            this.read = (packet) => packet.readBufferLengthEncoded();
            break;
          }
          ...
          this.read = (packet) => packet.readStringLength();
      }
    }
  }
