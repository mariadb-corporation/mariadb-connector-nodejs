...
class Pool extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.on('validateSize', this.#sizeHandler);
   ...
   */
  #sizeHandler() {
    if (this.#shouldCreateMoreConnections() && !this.#sizeHandlerTimeout) {
        ...
        function () {
          ...
          new Promise((resolve, reject) => {
            ...
            .catch((err) => {
              ...
              //delay next try
              this.#sizeHandlerTimeout = setTimeout(
                function () {
                  this.#sizeHandlerTimeout = null;
  ...
  #requestTimeoutHandler() {
    ...
    while ((request = this.#requests.peekFront())) {
      if (request.timeout <= currTime) {
        ...
        this.#requestTimeoutId = setTimeout(this.#requestTimeoutHandler.bind(this), request.timeout - currTime);
        return;
      }
    }
  }
  ...
  async #createConnection() {
    ...
    conn.release = function (resolve, release) {
      ...
      new Promise(revertFunction).then(pool.release.bind(pool, conn), pool.#destroy.bind(pool, conn)).finally(resolve);
    };
   ...
   */
    ...
    this.#closed = true;
    clearInterval(this.#unusedConnectionRemoverId);
    clearInterval(this.#sizeHandlerTimeout);
