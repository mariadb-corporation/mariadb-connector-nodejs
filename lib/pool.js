...
function Pool(options, useCallback) {
  ...
  const useConnectionPromise = function(conn, sql, values, isBatch) {
    if (sql) {
      const fct = isBatch ? conn.query : conn.batch;
        ...
        if (closed) {
          ...
          overlayNewConnection(conn, pool, function(conn, self) {
            ...
            conn.end = () => {
              ...
              if ((conn.info.isMariaDB() && conn.info.hasMinVersion(10,2,4)) || (!conn.info.isMariaDB()&& conn.info.hasMinVersion(5,7,3))) {
  ...
  const handleTaskQueue = function() {
    ...
    if (task) {
      ...
      if (conn) {
        ...
        //handle task
        if (task.sql) {
          const fct = task.isBatch ? conn.query : conn.batch;
          if (useCallback) {
