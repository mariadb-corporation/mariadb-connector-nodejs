...
function Pool(options, useCallback) {
  ...
  const useConnectionPromise = async function(conn, sql, values) {
    if (sql) {
      try {
  ...
  const useConnectionCallback = async function(conn, sql, values) {
    if (sql) {
      return new Promise((resolve, reject) => {
   ...
   */
  const addConnectionToPoolPromise = async function(pool) {
