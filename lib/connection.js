...
class Connection extends EventEmitter {
  ...
  connect() {
    ...
    switch (this.status) {
      ...
      case Status.AUTHENTICATING:
        ...
        this.opts.logger.error?.(errAuth);
  ...
  batch(_sql, _options, _values) {
    ...
    return new Promise(this.prepare.bind(this, _options, _sql, this.executePromise.bind(this))).then((prepare) => {
      ...
      let useBulk = this.#canUseBulk(vals, _options);
      if (useBulk) {
        ...
        killCon
          ...
          .then(function () {
            ...
            const killResHandler = function (err) {
              ...
              self.opts.logger.error?.(destroyError);
            ...
            new Promise(killCon.query.bind(killCon, null, `KILL ${self.info.threadId}`, undefined).bind(this)).finally(
              killResHandler
            );
          })
          .catch((err) => {
  ...
  createSecureContext(rejected, callback) {
    const socketError = this.socketErrorHandler.bind(this, function (err) {
      this.opts.logger.error?.(err);
      rejected(err);
    });
