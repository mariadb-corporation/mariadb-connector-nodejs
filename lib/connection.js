...
function Connection(options) {
  ...
  this._executePromise = (prepare, values) => {
    return new Promise(function (resolve, reject) {
      const cmd = new Execute(resolve, reject, null, opts, prepare, values);
      if (opts.trace) Error.captureStackTrace(cmd);
      _addCommand(cmd);
    });
  };
  ...
  this._prepareAndExecutePromise = (sql, values) => {
    ...
    return this._preparePromise(_sql).then((prepare) => {
      return new Promise(function (resolve, reject) {
  ...
  this._preparePromise = function (sql) {
    const self = this;
    return new Promise(function (resolve, reject) {
      const cmd = new Prepare(resolve, reject, opts, sql, self);
      if (opts.trace) Error.captureStackTrace(cmd);
      _addCommand(cmd);
    });
  };

  // this.unprepare = function (sql) {
  //   if (sql && opts.prepareCacheLength > 0) {
  //     if (opts.namedPlaceholders) {
  //       const res = Parse.searchPlaceholder(sql);
  //       sql = res.sql;
  //     }
  //     const key = info.database + '|' + sql;
  //     const prepare = info._prepareCache.get(key);
  //     if (prepare) prepare.close();
  //   }
  // };
