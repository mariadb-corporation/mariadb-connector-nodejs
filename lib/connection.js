...
function Connection(options) {
  ...
  this.batch = (sql, values) => {
    ...
    return new Promise(function(resolve, reject) {
      const cmd = new Batch(resolve, reject, _options, _sql, values);
      if (opts.trace) Error.captureStackTrace(cmd);
      _addCommand(cmd);
    });
  };
  ...
  this.reset = () => {
    ...
    return Promise.reject(new Error("Reset command not permitted for server " + info.serverVersion));
  };
    ...
    if (opts.sessionVariables) {
      ...
      if (keys.length > 0) {
        return new Promise(function(resolve, reject) {
          ...
          const cmd = new Query(resolve, errorHandling, null, sessionQuery, values);
          if (opts.trace) Error.captureStackTrace(cmd);
          _addCommand(cmd);
        });
      }
      ...
      initialArr.forEach(sql => {
        initialPromises.push(
          new Promise(function(resolve, reject) {
            ...
            const cmd = new Query(resolve, errorHandling, null, sql, null);
            if (opts.trace) Error.captureStackTrace(cmd);
            _addCommand(cmd);
          })
        );
