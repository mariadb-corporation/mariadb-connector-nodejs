          ...
          .then(
            function (res) {
              if (cmdParam.opts && cmdParam.opts.fullResult) {
                ...
                if (
                  (cmdParam.opts && cmdParam.opts.metaAsArray) ||
                  (cmdParam.opts && cmdParam.opts.metaAsArray === false && this.opts.metaAsArray)
                )
                ...
                if (firstResult instanceof OkPacket) {
                  ...
                  // aggregate results
                  if (cmdParam.opts.metaAsArray || (cmdParam.opts.metaAsArray === false && this.opts.metaAsArray)) {
                    const rs = [];
                    res.forEach((row) => {
                      rs.push(...row[0]);
                    });
