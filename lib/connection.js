...
class Connection extends EventEmitter {
  ...
  executeSessionVariableQuery() {
    if (this.opts.sessionVariables) {
      ...
      if (keys.length > 0) {
        ...
        return new Promise(this.query.bind(this, null, sessionQuery, values)).catch((initialErr) => {
  ...
  checkServerTimezone() {
    ...
    if (this.opts.tz && !this.opts.skipSetTimezone) {
      ...
      return new Promise(this.query.bind(this, null, 'SET time_zone=?', [tz])).catch((err) => {
        if (this.opts.logger.error) this.opts.logger.error(err);
        console.log(
