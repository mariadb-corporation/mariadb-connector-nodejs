...
class Connection extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.info = new ConnectionInformation(this.opts, this.redirect.bind(this), this.fatalError.bind(this));
    this.prepareCache =
    ...
    this._boundSendNextCmd = this._sendNextCmdImmediate.bind(this);
      ...
      .then(
        (chunkResults) => {
          ...
          if (chunkIndex === totalBatches - 1) {
            const cmdOpt = Object.assign(Object.create(this.opts), cmdParam.opts);
   ...
   */
    ...
    if (this.status < Status.TERMINATING_SOCKET && this.status !== Status.NOT_CONNECTED) {
      this.status = Status.TERMINATING_SOCKET;
      const ended = () => {
        this.status = Status.CLOSED;
        this.socket.destroy();
        this.socket.unref();
  ...
  destroy() {
    this.addCommand = this.addCommandDisabled;
    clearTimeout(this.timeout);
    if (this.status < Status.TERMINATING_SOCKET) {
      this.status = Status.TERMINATING_SOCKET;
      this.sendQueue.clear();
      if (this.activeReceiveCmd()) {
   ...
   */
  authSucceedHandler() {
    this.streamIn.maxAllowedPacket = this.opts.maxAllowedPacket;
  ...
  createSecureContext(info, callback) {
    ...
    // The self-signed fingerprint handshake applies to a MariaDB >= 11.4.1 server reached over a
    // non-unix-socket connection, with a password and no pinned CA: the server relaxes certificate
    // verification and authenticates its self-signed certificate with a fingerprint hash in the ending
    // OK packet. Deciding it once here (rather than computing a fingerprint on every TLS connection)
    info.useFingerprintValidation =
      info.requireValidCert &&
      Boolean(this.opts.password) &&
      !(this.opts.ssl && this.opts.ssl.ca) &&
      !this.opts.socketPath &&
      info.isMariaDB() &&
      info.hasMinVersion(11, 4, 1);

    const baseConf = { socket: this.socket };

    if (info.useFingerprintValidation) {
      // permit the self-signed certificate at TLS level; it is authenticated afterwards by the
      // fingerprint hash in the ending OK_PACKET (handshake.js / authentication.js).
  ...
  unexpectedPacket(packet) {
    if (packet && packet.peek() === 0xff) {
      ...
      if (err.fatal && this.status < Status.TERMINATING) {
    ...
    } else if (this.status < Status.TERMINATING) {
      this.status = Status.TERMINATING;
      const err = Errors.createFatalError(
  ...
  /**
   * First command of the receive queue still expecting packets, discarding the ones already ended.
   * To be used in place of a plain queue emptiness check whenever the question is "is a command
   * currently running", so a command already ended is never mistaken for one still in progress.
   *
   * @returns {Object|null} command waiting for packets, null when none is active
   * @private
   */
  activeReceiveCmd() {
    let cmd;
  ...
  addCommandEnable(cmd, expectResponse) {
    cmd.once('end', this._boundSendNextCmd);

    //send immediately only if no current active receiver
    if (this.sendQueue.isEmpty() && !this.activeReceiveCmd()) {
  ...
  addCommandEnablePipeline(cmd, expectResponse) {
    cmd.once('send_end', this._boundSendNextCmd);
  ...
  socketErrorHandler(err) {
    if (this.status >= Status.TERMINATING_SOCKET) return;
    if (this.socket) {
  ...
  socketErrorDispatchToQueries(err) {
    ...
    while ((receiveCmd = this.receiveQueue.shift())) {
      // Notify commands that have not ended, including commands that have not started yet.
      if (receiveCmd && receiveCmd.onPacketReceive !== null) {
  ...
  changeUser(cmdParam, resolve, reject) {
    ...
    // COM_CHANGE_USER resets the session server-side; on failure (e.g. authentication error)
    // the connection is no longer usable, so tear it down and reject with the actual error.
    // (authFailHandler only rejects the initial-connect promise, so it must not be used as the
    // command reject here, otherwise the error would be silently swallowed and the connection
    // left hanging.)
    const onChangeUserError = (err) => {
      if (err.fatal) {
        conn.fatalError(err, true);
      } else {
        conn.end(
          () => {},
          () => {}
    ...
    this.addCommand(
      new ChangeUser(
        ...
        this.opts,
        (res) => {
          if (conn.status < Status.TERMINATING && conn.opts.pipelining) conn.addCommand = conn.addCommandEnablePipeline;
  ...
  prepare(cmdParam, resolve, reject) {
    ...
    if (this.prepareCache && (this.sendQueue.isEmpty() || !this.activeReceiveCmd())) {
    ...
    return (
      ...
      return searchDbPromise.then(() => {
        const endingFunction = () => {
          if (conn.status < Status.TERMINATING) {
            conn.addCommand = conn.addCommandEnable.bind(conn);
            if (conn.status < Status.TERMINATING && conn.opts.pipelining) {
        ...
        // cmdParam.file is the path supplied by the application through importFile();
        // it is a client-side API argument, not data received from the server.
  ...
  redirect(value, resolve) {
    if (this.opts.permitRedirect && value) {
      ...
      this.activeReceiveCmd(); // so the length below only counts commands still receiving
      if (this.receiveQueue.length <= 1 && (this.info.status & ServerStatus.STATUS_IN_TRANS) === 0) {
        ...
        if (!url || (url.protocol !== 'mariadb:' && url.protocol !== 'mysql:')) {
        ...
        const options = {
          host: decodeURIComponent(ConnectionOptions.unwrapIpv6(url.hostname)),
