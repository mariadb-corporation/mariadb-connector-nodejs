...
function Connection(options) {
  ...
  this.batch = (sql, values) => {
    ...
    return new Promise(function(resolve, reject) {
      const cmd = new BatchRewrite(resolve, reject, _options, opts, _sql, values);
      if (opts.trace) Error.captureStackTrace(cmd);
      _addCommand(cmd);
    });
  };
  ...
  this._batchCallback = (sql, values, cb) => {
    ...
    if (_cb) {
      ...
      cmd = new BatchRewrite(resolve, _cb, _cmdOpts, opts, _sql, _values);
    } else {
      cmd = new BatchRewrite(() => {}, () => {}, _cmdOpts, opts, _sql, _values);
    }
    cmd.handleNewRows = row => {
      cmd._rows[cmd._responseIndex].push(row);
      cmd.emit("data", row);
