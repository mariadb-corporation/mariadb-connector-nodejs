...
function Connection(options) {
      ...
      return Promise.reject(
        Errors.createError(
          "Query values cannot be a function. " +
            "If wanting to use callback, use import 'mariadb/callback'",
          true,
          info,
          "08S01",
  ...
  /**
   * Execute query returning a Readable Object that will emit data/end/error events.
   * to permit streaming big resultset
   *
   * @param sql     sql parameter Object can be used to supersede default option.
   *                Object must then have sql property.
   * @param values  object / array of placeholder values (not mandatory)
  ...
  const _initSocket = authFailHandler => {
    ...
    if (opts.connectTimeout) {
      this.timeout = setTimeout(
        _connectTimeoutReached.bind(this),
        opts.connectTimeout,
        authFailHandler
      );
    }
    ...
    _socket.on(
      ...
      function() {
        ...
        if (_status === Status.CONNECTING) {
          ...
          // if (opts.connectTimeout) _socket.removeAllListeners("timeout");
