...
class Connection extends EventEmitter {
  ...
  batch(cmdParam, resolve, reject) {
    ...
    this.prepare(cmdParam, (prepare) => this.executeBatch(cmdParam, prepare, resolve, reject), reject);
  }
  ...
  formatBatchValues(values, usePlaceHolder, parameterCount) {
    ...
    if (parameterCount === 1) {
      // Convert [a, b, c] to [[a], [b], [c]] when only one parameter
      const result = [];
  ...
  aggregateResultSets(results, cmdOpt, resolve) {
    if (cmdOpt.metaAsArray) {
      ...
      const rs = Array(totalLength);

      let index = 0;
      for (const row of results) {
        if (row[0] && row[0].length) {
          for (const item of row[0]) {
            rs[index++] = item;
          }
        }
      }
      ...
      Object.defineProperty(rs.slice(0, index), 'meta', {
        value: results[0].meta,
        writable: true,
        enumerable: cmdOpt.metaEnumerable
      });

      resolve(rs.slice(0, index));
    }
  }
  ...
  _canUseBulk(values, _options) {
    if (_options && _options.fullResult && (this.info.clientCapabilities & Capabilities.BULK_UNIT_RESULTS) == 0n)
      return false;
  ...
  socketInit() {
    ...
    if (this.opts.connectTimeout) {
      this.socket.setTimeout(this.opts.connectTimeout, this.socketTimeoutReached.bind(this));
    }
