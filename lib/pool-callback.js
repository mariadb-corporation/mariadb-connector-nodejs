...
function PoolCallback(options) {
  const processTaskCallback = function (conn, sql, values, type) {
    if (sql) {
      return new Promise((resolve, reject) => {
        const fct = type === PoolBase.QUERY ? conn.query : type === PoolBase.EXECUTE ? conn.execute : conn.batch;
   ...
   * @returns {Query} query
   */
  const _executeCallback = function (sql, values, cb) {
    let _cb = cb,
      _values = values;
