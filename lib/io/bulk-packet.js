...
class BulkPacket {
  constructor(opts, out, row) {
    ...
    this.writeBinaryDate =
      opts.timezone === "local" ? this.writeBinaryLocalDate : this.writeBinaryTimezoneDate;
  ...
  writeLengthStringAscii(val) {
    ...
    if (len >= this.buf.length - this.pos) {
      ...
      this.writeLengthEncodedBuffer(strBuf);
      return;
    }
  ...
  writeLength(len) {
    if (len < 0xfb) {
      ...
      this.buf[this.pos++] = len >>> 32;
      this.buf[this.pos++] = len >>> 40;
      this.buf[this.pos++] = len >>> 48;
      this.buf[this.pos++] = len >>> 56;
    }
  }

  writeLengthEncodedBuffer(val) {
    let valLen = val.length;
    this.writeLength(valLen);
  ...
  writeInt16(value) {
    if (this.pos + 2 >= this.buf.length) {
      if (this.buf.length < this.maxPacketSize) this.growBuffer(2);
      if (this.pos + 2 >= this.buf.length) {
  ...
  writeInt24(value) {
    if (this.pos + 3 >= this.buf.length) {
      if (this.buf.length < this.maxPacketSize) this.growBuffer(3);
      if (this.pos + 3 >= this.buf.length) {
  ...
  writeInt32(value) {
    if (this.pos + 4 >= this.buf.length) {
      if (this.buf.length < this.maxPacketSize) this.growBuffer(4);
      if (this.pos + 4 >= this.buf.length) {
    ...
    this.buf[this.pos++] = value >>> 8;
    this.buf[this.pos++] = value >>> 16;
    this.buf[this.pos++] = value >>> 32;
  }

  writeIconvLengthEncodedString(str) {
    let buf = Iconv.encode(str, this.encoding);
    this.writeLengthEncodedBuffer(buf, 0, buf.length);
  }
  ...
  writeDefaultLengthEncodedString(str) {
    ...
    if (str.length * 3 + 10 < this.buf.length - this.pos) {
      ...
      if (maxlen < 0xfb) {
        ...
        this.buf[lengthPos + 1] = realLen >>> 8;
        this.buf[lengthPos + 2] = realLen >>> 16;
        this.buf[lengthPos + 3] = realLen >>> 24;
        this.buf[lengthPos + 4] = realLen >>> 32;
        this.buf[lengthPos + 5] = realLen >>> 40;
        this.buf[lengthPos + 6] = realLen >>> 48;
        this.buf[lengthPos + 7] = realLen >>> 56;
      }
      return;
    }
    ...
    if (byteLength + 9 > this.buf.length - this.pos) {
      if (this.buf.length < MAX_BUFFER_SIZE) this.growBuffer(byteLength);
  ...
  writeBinaryLocalDate(date, opts) {
    ...
    this._writeBinaryDate(year, mon, day, hour, min, sec, ms);
  }
  ...
  _writeBinaryDate(year, mon, day, hour, min, sec, ms) {
    ...
    //not enough space remaining
    if (len >= this.buf.length - this.pos) {
      let tmpBuf = Buffer.allocUnsafe(len);
      ...
      this.writeLengthEncodedBuffer(tmpBuf);
      return;
    }
