 ...
 */
class WritePacket {
  constructor(out, initString, endString) {
    this.out = out;
    this.buf = Buffer.allocUnsafe(SMALL_BUFFER_SIZE);
    this.pos = 4;
    this.end = SMALL_BUFFER_SIZE;
    ...
    MAX_PACKET_SIZE = MAX_BUFFER_SIZE - this.endStrLength;
  ...
  writeBuffer(arr, off, len) {
    if (len > this.buf.length - (this.pos + this.endStrLength)) {
      ...
      if (len > this.buf.length - (this.pos + this.endStrLength)) {
        ...
        while (true) {
          //filling buffer
          let lenToFillBuffer = Math.min(MAX_PACKET_SIZE - this.pos, remainingLen);
          // console.log("lenToFillBuffer:" + lenToFillBuffer);
  ...
  mark(isLast) {
    if (this.singleQuery) {
      ...
      if (this.markPos && this.pos + this.endStrLength > MAX_BUFFER_SIZE) {
  ...
  flush(end, remainingLen) {
    // console.log("flush mark:" + this.markPos + " " + this.singleQuery)
    if (this.markPos && !this.singleQuery) {
      ...
      this.out.flushBuffer(end, remainingLen);
      this.pos = 4;
      this.markPos = undefined;
  ...
  flushMark() {
    ...
    // console.log(this.buf.slice(0, this.markPos).toString('utf8').substring(16770000));
    this.out.buf = this.buf;
    this.out.pos = this.pos;
