...
class PacketOutputStream {
  ...
  rewriteEndPart(str) {
    ...
    let lastIndex = 0;
    while ((match = CHARS_GLOBAL_REGEXP.exec(str)) !== null) {
      this.batchEndPart += str.slice(lastIndex, match.index) + '/' + match[0].charCodeAt(0);
      lastIndex = CHARS_GLOBAL_REGEXP.lastIndex;
    }

    if (lastIndex === 0) {
      // Nothing was escaped
      this.batchEndPart += str;
      return;
    }

    if (lastIndex < str.length) {
      this.batchEndPart += str.slice(lastIndex);
    }
    this.batchEndPartLength = Buffer.byteLength(str, this.encoding);
  }
  ...
  resetMark() {
    ...
    this.batchEndPartLength = undefined;
    this.batchEndPart = undefined;
  }
}
