...
import { getMbRecognizer } from '../misc/charset-mb.js';
import { DEFAULT_MAX_ALLOWED_PACKET } from '../config/connection-options.js';
...
const QUOTE_GLOBAL_REGEXP = /'/g;

const ESCAPE_BACKSLASH = new Uint8Array(256);
ESCAPE_BACKSLASH[QUOTE] = SLASH;
ESCAPE_BACKSLASH[DBL_QUOTE] = SLASH;
ESCAPE_BACKSLASH[SLASH] = SLASH;
ESCAPE_BACKSLASH[ZERO_BYTE] = SLASH;

const ESCAPE_DOUBLE_QUOTE = new Uint8Array(256);
ESCAPE_DOUBLE_QUOTE[QUOTE] = QUOTE;
...
class PacketOutputStream {
  constructor(opts, info) {
    ...
    this.maxAllowedPacket = opts.maxAllowedPacket || DEFAULT_MAX_ALLOWED_PACKET;
  ...
  changeEncoding(collation) {
    ...
    // For multi-byte charsets whose trail-byte range overlaps the ASCII escape
    // character (0x5C), the buffer escape must be charset-aware to avoid the
    // server lexer eating the inserted backslash as part of a multi-byte char
    const mb = getMbRecognizer(this.encoding);
    this.writeBufferEscape = mb ? this.writeBufferEscapeMb.bind(this, mb) : this.writeBufferEscapeFast;
  }
  ...
  /**
   * Current session escape mode. When the server runs with NO_BACKSLASH_ESCAPES,
   * backslash is an ordinary character and a quote is escaped by doubling it.
   * Escaping with a backslash in that mode leaves the quote closing the literal,
   * making the value injectable. Same behaviour as Connector/C and Connector/J.
   *
   * @returns {boolean} true when quotes must be doubled instead of backslashed
   */
  noBackslashEscapes() {
    return (this.info.status & ServerStatus.STATUS_NO_BACKSLASH_ESCAPES) !== 0;
  }

  /**
   * Escape table for the current session mode, resolved once per value so the
   * per-character loops stay free of any mode test.
   *
   * @returns {Uint8Array} byte to prepend per byte value, 0 when none
   */
  escapeTable() {
    return this.noBackslashEscapes() ? ESCAPE_DOUBLE_QUOTE : ESCAPE_BACKSLASH;
  }
  ...
  writeUtf8StringEscapeQuote(str) {
    ...
    //not enough space remaining
    if (this.noBackslashEscapes() || charsLength * 3 + 2 >= this.buf.length - this.pos) {
  ...
  writeDefaultStringEscapeQuote(str) {
    this.writeInt8(QUOTE);
    const noBackslashEscapes = this.noBackslashEscapes();
    const escapeRegexp = noBackslashEscapes ? QUOTE_GLOBAL_REGEXP : CHARS_GLOBAL_REGEXP;
    const escapeChar = noBackslashEscapes ? QUOTE : SLASH;
    let match;
    let lastIndex = 0;
    while ((match = escapeRegexp.exec(str)) !== null) {
      this.writeString(str.slice(lastIndex, match.index));
      this.writeInt8(escapeChar);
  ...
  // Default dispatch; replaced in changeEncoding() by either
  // writeBufferEscapeFast (single-byte / utf8 / utf16 / utf32 / ucs2 / euckr / gb2312 / ujis / eucjpms charsets)
  // or writeBufferEscapeMb bound to a charset-specific recognizer
  // (big5 / gbk / sjis / cp932 / gb18030).
  writeBufferEscape(val) {
    this.writeBufferEscapeFast(val);
  }

  writeBufferEscapeFast(val) {
    let valLen = val.length;
    const escapeTable = this.escapeTable();
    ...
    for (let i = 0; i < valLen; i++) {
      const escape = escapeTable[val[i]];
      if (escape !== 0) this.buf[this.pos++] = escape;
      this.buf[this.pos++] = val[i];
    }
  }

  // Charset-aware buffer escape for multi-byte client charsets where a trail
  // byte may collide with the ASCII escape character (0x5C). When the next two
  // bytes form a valid mb char, copy them verbatim so no escape is inserted
  // between them. A lone head byte (no valid trail) is itself escaped (\<head>)
  // so it cannot combine with whatever follows on the wire. This mirrors the
  // server lexer's own behaviour: my_ismbchar runs before escape processing,
  // and a \X sequence emits X as a literal byte.
  //
  // Note on streaming: this is called per chunk from query.js when a Readable
  // is passed as a parameter. A multi-byte char that straddles a chunk boundary
  // ends up with the head escaped in chunk N and the trail emitted plainly in
  // chunk N+1. That is safe but may corrupt mb-encoded payloads streamed in
  // tiny chunks under big5/gbk/sjis/cp932/gb18030. Buffer-typed parameters
  // (the common path) are passed as a single buffer and are unaffected.
  writeBufferEscapeMb(mb, val) {
    // With NO_BACKSLASH_ESCAPES the charset-aware walk is unnecessary: no
    // backslash is ever emitted, so no head byte can absorb one, and the quote
    // (0x27) sits below the trail-byte range of every charset handled here, so
    // it can never be absorbed into a multi-byte char. Doubling it byte-wise is
    // both safe and cheaper.
    if (this.noBackslashEscapes()) return this.writeBufferEscapeFast(val);

    const valLen = val.length;
    // Worst case is 2 output bytes per input byte (escape inserted) — same
    // upper bound as the fast path.
    if (valLen * 2 > this.buf.length - this.pos) {
      if (this.buf.length !== MAX_BUFFER_SIZE) this.growBuffer(valLen * 2);
    }
    ...
    while (i < valLen) {
      ...
      if (mb.isHead(b)) {
        ...
        // Lone head byte: escape it so it cannot combine with the following
        // byte on the wire.
        if (this.pos >= this.buf.length) this.flushBuffer(false, (valLen - i) * 2);
        this.buf[this.pos++] = SLASH;
        if (this.pos >= this.buf.length) this.flushBuffer(false, (valLen - i) * 2);
  ...
  checkMaxAllowedLength(length, info) {
    if (this.cmdLength + length >= this.maxAllowedPacket) {
      ...
      return Errors.createError(
        ...
        Errors.client.ER_MAX_ALLOWED_PACKET,
        info
      );
    }
    return null;
  ...
  fastFlushDebug(cmd, packet) {
    ...
    this.opts.logger.network(
      ...
      }\n${log(this.opts, packet, 0, packet.length)}`
    );
    this.cmdLength = 0;
    this.markPos = -1;
  }
...
export default PacketOutputStream;
