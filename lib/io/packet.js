...
class Packet {
  ...
  readStringNullEnded() {
    ...
    return this.buf.toString(fasterBufferEncoding('utf8'), initialPosition, initialPosition + cnt);
  }
  ...
  readError(info, sql, stack) {
    ...
    if (this.peek() === 0x23) {
      // skip '#'
      this.skip(6);
      sqlState = this.buf.toString(fasterBufferEncoding('utf8'), this.pos - 5, this.pos);
      msg = this.readStringNullEnded();
    } else {
      // pre 4.1 format
      sqlState = 'HY000';
      msg = this.buf.toString(fasterBufferEncoding('utf8'), this.pos, this.end);
    }
