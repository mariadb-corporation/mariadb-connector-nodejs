...
class ConnectionPromise {
  ...
  static _EXECUTE_CMD(conn, cmdParam) {
    ...
    if ((prepareFromCache = conn.info.prepareFromCache(cmdParam.sql)) != null) {
      return prepareFromCache.execute(cmdParam.values, cmdParam.opts, null, cmdParam.stack)
          .finally(() => prepareFromCache.close());
    }
    return new Promise(conn.prepare.bind(conn, cmdParam))
      .then((prepare) => {
        return prepare.execute(cmdParam.values, cmdParam.opts, null, cmdParam.stack)
